...
def visualize_expenses():
    global canvas
    global cursor
    cursor.execute("SELECT Description, SUM(Amount) FROM ExpenseTracker GROUP BY Description")
    rows = cursor.fetchall()
    Description = [row[0] for row in rows]
    Amount = [row[1] for row in rows]
    
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(Amount, labels=Description, autopct='%1.1f%%', startangle=140)
    ...
    exit_button = Button(root, text='Exit', command=hide_pie_chart)
...
pie_chart_button = Button(root, text='Visualization', command=visualize_expenses)
pie_chart_button.pack(side=BOTTOM, pady=10)
